...
public class RandomValue {
    ...
    private static String num = "123456789";
     ...
     * @return
     */
    public static String getCardIdBuType(int cardType){
        StringBuilder sb = new StringBuilder();
        Random random = new Random();

        if(cardType == 0){
            //储虚卡，19位，前六位固定，中间十二位随机，最后一位检校码
            sb.append("622200");
            for (int i = 0; i < 12; i++) {
    ...
    public static  String getaddr(){
        Random random = new Random();
        int randomInt = random.nextInt(10000)%Cities.addrs.length;

        return Cities.addrs[randomInt];
    }

    private static void testRondomDate() {
        Date date = getRandomDate("2009-07-01","2019-07-01");
